...
files_modified:
  ...
  - tests/fixtures/test_training_config.yaml
autonomous: false

must_haves:
  truths:
    - "小规模端到端训练（50条GRPO、20条SFT、10步）成功完成"
    - "完整四步流程全部执行：GRPO数据生成 → SFT数据生成 → SFT训练 → GRPO训练"
  ...
  artifacts:
    ...
    - path: "scripts/run_integration_test.sh"
      provides: "集成测试执行脚本"
      min_lines: 60
    - path: "tests/fixtures/test_training_config.yaml"
      provides: "测试专用训练配置"
      contains: "dataset_path", "output_dir", "num_train_steps"
...
测试数据配置：tests/fixtures/test_training_config.yaml
- 配置dataset_path指向tests/fixtures/testdata/
- 配置output_dir指向临时训练目录
- 设置小规模训练参数（num_train_steps=10, SFT训练步数=5）
</context>
...
1. test_end_to_end_training_small_scale：
   ...
   - 运行完整四步流程：GRPO数据生成 → SFT数据生成 → SFT训练 → GRPO训练
   - SFT训练：20条数据，少量训练步（如5-10步）
   - GRPO训练：50条数据，10步训练验证
   - 验证模型输出文件存在
   - 验证训练日志无ERROR
   - 验证训练时间在合理范围内（<30分钟）
...
3. 测试流程（完整四步）：
   a) 准备阶段：
      - 确保测试数据存在
      - 创建临时输出目录
      - 配置测试专用训练配置（--config tests/fixtures/test_training_config.yaml）

   b) GRPO数据生成阶段：
      - 调用grpo/generate_grpo_dataset.py
      - 生成50条测试GRPO数据

   c) SFT数据生成阶段：
      - 从GRPO数据生成20条SFT数据
      - 调用grpo/generate_sft_dataset.py

   d) SFT训练阶段（小规模，不跳过）：
      - 调用grpo/sft_training.py
      - 使用20条SFT数据
      - 训练5-10步验证流程
      - 输出SFT检查点
...
4. 数据传递配置：
   - 创建tests/fixtures/test_training_config.yaml
   - 配置dataset_path指向测试数据目录
   - 配置output_dir指向temp_training_dir
   - 设置小规模训练参数（num_train_steps=10等）
...
3. 训练日志无ERROR
4. 测试执行时间 < 30分钟
5. 四步流程全部执行（GRPO数据生成 → SFT数据生成 → SFT训练 → GRPO训练）
  </verify>
  <done>
端到端训练测试成功完成完整四步流程，模型文件输出正确，训练日志无ERROR。
  </done>
</task>
...
3. 完整四步流程全部执行：GRPO数据生成 → SFT数据生成 → SFT训练 → GRPO训练
4. 训练输出模型文件可加载
5. 训练日志无ERROR，包含reward统计
6. 整个测试流程在30分钟内完成
7. run_integration_test.sh脚本可一键执行所有集成测试
</success_criteria>
