...
must_haves:
  truths:
    ...
    - "训练脚本使用日期命名保存模型到outputs/YYYY-MM-DD_sft/和outputs/YYYY-MM-DD_grpo/"
  artifacts:
  ...
  key_links:
    ...
    - from: "grpo.sft_training"
      to: "outputs/YYYY-MM-DD_sft/"
      via: "SFT模型保存使用日期命名"
      pattern: "outputs.*\\d{4}-\\d{2}-\\d{2}_sft"
    - from: "grpo.training"
      to: "outputs/YYYY-MM-DD_grpo/"
      via: "GRPO模型保存使用日期命名"
      pattern: "outputs.*\\d{4}-\\d{2}-\\d{2}_grpo"
---
...
4. 模型路径使用日期格式YYYY-MM-DD（从训练开始日期获取）
  </action>
  <verify>
运行完整训练后检查摘要输出
  </verify>
  <done>
训练摘要显示具体的SFT模型路径(outputs/YYYY-MM-DD_sft/)和GRPO模型路径(outputs/YYYY-MM-DD_grpo/)，以及数据集大小和训练时间
  </done>
</task>
